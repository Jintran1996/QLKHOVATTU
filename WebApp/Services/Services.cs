...
namespace ToolsApp.Services
{
    public class NetSuiteItemFulfillmentService
    {
        ...
        public static bool CreateYarnSalesOrderFulfillment(string SO, string so_internalId, string soctxn, DateTime ngayct, List<NL_CTXUATNHAPViewModels> lines)
        {
            NSClient.NSClient _nsClient = new NSClient.NSClient();
            InitializeRecord ir = new InitializeRecord();
            ir.type = InitializeType.itemFulfillment;
            ...
            if (getInitResponse.status.isSuccess)
            {
                ...
                // TẠO DANH SÁCH TẠM: Sao chép dữ liệu từ danh sách lines ban đầu để thực hiện loại trừ
                List<NL_CTXUATNHAPViewModels> remainingLines = lines != null
                    ? new List<NL_CTXUATNHAPViewModels>(lines)
                    : new List<NL_CTXUATNHAPViewModels>();
                for (int i = 0; i < ifitemList.item.Length; i++)
                {
                    ItemFulfillmentItem ffItem = new ItemFulfillmentItem();
                    ffItem.item = ifitemList.item[i].item;
                    ...
                    var currentNsItem = ifitemList.item[i]; // Gán biến cụ thể tránh viết lặp lại


                    if (lines != null && currentNsItem.item != null)
                    {
                        var ctitem = remainingLines.FirstOrDefault(x =>
          x.MAVT == currentNsItem.item.name &&
          x.SOLUONGTT == (decimal)currentNsItem.quantityRemaining);

                        // 3. Kiểm tra ctitem khác null để tránh lỗi NullReferenceException
                        if (ctitem != null)
                        {
                            ffItem.inventoryDetail = BuildInventoryDetail(ctitem);

                            remainingLines.Remove(ctitem);
                        }
                    }
        ...
        private static InventoryDetail BuildInventoryDetail(NL_CTXUATNHAPViewModels ctitem)
        {
            ...
            assignment.issueInventoryNumber = lotRecord;

            assignment.quantity = Convert.ToDouble(ctitem.SOLUONGTT);

            assignment.quantitySpecified = true;

            InventoryAssignmentList list = new InventoryAssignmentList();

            list.inventoryAssignment = new[] {assignment};

            InventoryDetail detail = new InventoryDetail();

            detail.inventoryAssignmentList = list;
