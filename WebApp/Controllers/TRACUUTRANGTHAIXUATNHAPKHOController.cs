...
{
    public class TRACUUTRANGTHAIXUATNHAPKHOController : BaseController
    {
        #region Database
        ...
        public ActionResult GetList(SearchTraCuuXNKho src)
        {
            ...
            var tuNgay = src.TUNGAY_;
            var denNgay = src.DENNGAY_;
           
            string maKhoCsv = (listKho != null && listKho.Any())
                                ? string.Join(",", listKho)
                                : "";

            var list = dbvt.VATTU2025_SP_LOAD_PHIEUXNKHO_DONGBO_NETSUITE(maKhoCsv, tuNgay, denNgay)
                           .ToList();
